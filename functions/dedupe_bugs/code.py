# Function:   dedupe_bugs
# Description: Cluster duplicate bug reports and return a draft merge per cluster. Structured-match first (stack-trace hash, component+error_code, normalized repro URL), with TF-IDF cosine and character-trigram Jaccard fallbacks. Output includes clusters, a canonical pick per cluster, and a draft merged bug (title, body with cross-references, deduped repro steps, union of normalized stack traces).
# Source:      Exported from the pod via `lemma --json functions get`.
# Hash:        ed998e8684fc122c6b01e1987c9972edf16398d2befc01e566035c7a1cc79fac
# Visibility:  POD


#input_type_name: DedupeBugsInput
#output_type_name: DedupeBugsResult
#function_name: dedupe_bugs
...
class DedupeBugsInput(BaseModel):
    """Input payload - a batch of bug reports to dedupe.

    Each bug dict may carry: id, title, body, error_code, component,
    stack_trace, repro_url, repro_steps, reporter, vote_count,
    comment_count, created_at. Anything missing is treated as empty.
    """
    bugs: list[dict] = Field(..., description="Bug reports to cluster")
    similarity_threshold: float = Field(
        0.30, ge=0.0, le=1.0,
        description="Cosine threshold for the text-similarity fallback",
    )


class DedupeBugsResult(BaseModel):
    """Dedup report - clusters (with draft merges), singletons, edges, stats."""
    clusters: list[dict]
    singletons: list[str]
    edges: list[dict]
    stats: dict
...
async def dedupe_bugs(ctx: FunctionContext, data: DedupeBugsInput) -> DedupeBugsResult:
    payload = dedupe(data.bugs, similarity_threshold=data.similarity_threshold)
    out = payload.to_dict()
    return DedupeBugsResult(
...
# ---------------- dedupe module inlined; stdlib-only -----------------
